...
import org.apache.kafka.rest.RestApiProto.RestApiMessage.Operation;
import org.apache.kafka.rest.metrics.HttpMetric;
...
public class KafkaRestApiHttpRequestTest {
    ...
    private KafkaRestApiHttpRequest getMockMessage(HttpMethod method, String id, boolean includeObsoleteDocHeader, String... deletes) {
        KafkaRestApiHttpRequest message = Mockito.mock(KafkaRestApiHttpRequest.class);
    ...
    public void testURIParsing() {
        ...
        KafkaRestApiHttpRequest req = new KafkaRestApiHttpRequest(HTTP_1_1, POST, uri);
        ...
        req = new KafkaRestApiHttpRequest(HTTP_1_1, POST, String.format("/%s/%s/%s", endpoint, namespace, id));
        ...
        req = new KafkaRestApiHttpRequest(HTTP_1_1, POST, String.format("/%s/%s", endpoint, namespace));
        assertEquals(null, req.getApiVersion());
        assertEquals(namespace, req.getNamespace());
        assertEquals(endpoint, req.getEndpoint());
    ...
    private void testMessage(String id, boolean doDelete, int expectedDeletes, int expectedCreates, String... deletes) throws Exception {
        ...
        KafkaRestApiHttpRequest mockMessage = getMockMessage(HttpMethod.POST, id, doDelete, deletes);
    ...
    public void testDelete() throws Exception {
        ...
        MessageEvent messageEvent = Mockito.mock(MessageEvent.class);
        Mockito.when(messageEvent.getChannel()).thenReturn(mockChannel);
        KafkaRestApiHttpRequest mockMessage = getMockMessage(HttpMethod.DELETE, "delete-id", false);
        Mockito.when(messageEvent.getMessage()).thenReturn(mockMessage);
